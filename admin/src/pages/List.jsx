  ...
  return (
    ...
    <div className='flex flex-col gap-2'>
        ...
        {/* ---List Table Title---*/}
          ...
          {
            list.map((item, index) => (
              <div key={index} className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 border text-sm border-gray-300 px-2 py-1'>
                <img src={item.image[0]} alt="" className='w-12 ' />
                <p className='text-center'>{item.name}</p>
                <p className='text-center'>{item.category}</p>
                <p className='text-center'>{currency} {item.price}.00</p>
